<div class="space-y-8">
    <section
        ...
        <div class="space-y-3">
            ...
            <h1 class="text-3xl font-semibold sm:text-4xl">Bimbingan Online</h1>
            <p class="max-w-2xl text-sm text-blue-100 sm:text-base">Kirim pesan konsultasi dan unggah file pendukung ke
                dosen pembimbing.</p>
        </div>
    </section>
    ...
    <section class="grid grid-cols-1 gap-6 xl:grid-cols-[320px_1fr]">
        <article class="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
            <h2 class="text-base font-semibold text-slate-900">Daftar Dosen</h2>
            <p class="mt-1 text-xs text-slate-500">Klik dosen untuk membuka thread.</p>

            <div class="mt-4 space-y-2">
                @forelse ($dosenList as $item)
                    <button type="button" wire:click="$set('selected_dosen_id', '{{ $item['id'] }}')"
                        class="w-full rounded-2xl border px-3 py-3 text-left transition {{ (string) $selected_dosen_id === (string) $item['id'] ? 'border-blue-300 bg-blue-50' : 'border-slate-200 hover:border-slate-300' }}">
        ...
        <article class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            <div class="border-b border-slate-200 pb-4">
                ...
                @if (!$chatEnabled)
                    <p class="mt-1 text-xs font-medium text-amber-600">
                        Chat nonaktif. Anda perlu hadir minimal 1 jadwal bimbingan dari dosen ini.
                    </p>
                @endif
            </div>
            ...
            <div class="mt-4 space-y-3 max-h-96 overflow-y-auto pr-1">
                @forelse ($messages as $msg)
                    @php $isMine = $msg->sender_role === 'mahasiswa'; @endphp
                    <div class="flex {{ $isMine ? 'justify-end' : 'justify-start' }}">
                        <div
                            class="max-w-[80%] rounded-2xl px-4 py-3 {{ $isMine ? 'bg-blue-600 text-white' : 'bg-slate-100 text-slate-700' }}">
                            <p class="text-[11px] {{ $isMine ? 'text-blue-100' : 'text-slate-500' }}">
                                {{ $isMine ? 'Anda' : $msg->dosen?->user?->name ?? 'Dosen' }} •
                            ...
                            @if ($msg->attachment)
                                <a href="{{ Storage::url($msg->attachment) }}" target="_blank"
                                    class="mt-2 inline-flex rounded-lg px-2 py-1 text-xs font-semibold {{ $isMine ? 'bg-blue-500 text-white' : 'bg-white text-blue-700' }}">
                                    Lihat lampiran
                                </a>
                            @endif
                        </div>
            ...
            <form wire:submit.prevent="kirim" class="mt-5 space-y-3 border-t border-slate-200 pt-4">
                ...
                <div>
                    <textarea wire:model="message" rows="3" placeholder="Tulis pesan..."
                ...
                <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <input type="file" wire:model="attachment"
                        class="block w-full rounded-2xl border border-slate-200 px-4 py-2 text-sm text-slate-900 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-100 sm:max-w-xs" />
                    <button type="submit" @disabled(!$chatEnabled)
                        class="rounded-2xl bg-blue-600 px-5 py-2.5 text-sm font-semibold text-white hover:bg-blue-500">
                        Kirim
                    </button>
                </div>
                @error('attachment')
